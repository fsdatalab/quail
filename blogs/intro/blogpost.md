...
**Train models for AI-SQL operators.** It would also be good to train specialized models for filters and joins. [Google's work on lightweight proxy models for AI-SQL](https://arxiv.org/abs/2603.15970) suggests small filter models can reduce cost substantially. For joins, Quail currently runs a *KV-aware nested loop join*: Quail reuses each anchor's KV, but recomputes every partner for each new anchor, so every document pair still requires new model work. We could instead train a dual encoder, which encodes each document independently, on labeled matching and nonmatching pairs. The training objective would assign high similarity to matching pairs and low similarity to nonmatching pairs. Quail could then encode each document once, index the representations from one relation, and search the index with representations from the other relation. The resulting execution would look more like a hash join than a nested loop join. A symmetric similarity function would give the same match score regardless of which relation Quail indexes. One systems question is how to run and train many specialized models alongside a larger model on the same GPU.
