...
**Use proxy models for query planner hints.** Proxy models need not be limited to evaluating predicates during execution. They could also provide planner hints. For example, predicted filter selectivities could help Quail choose the filter order. Predictions about which documents will reach later operators could help Quail decide which KV to retain. When used only as hints, proxy errors would affect runtime rather than query results.
