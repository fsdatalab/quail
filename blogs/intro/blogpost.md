...
**Train models for AI-SQL operators.** It would also be good to train specialized models for filters and joins. [Google's work on lightweight proxy models for AI-SQL](https://arxiv.org/abs/2603.15970) suggests small filter models can reduce cost substantially. In a hash join, the database builds an in-memory hash table over one relation, called the *build side*, then looks up matches for each row from the other relation, called the *probe side*. Each row is processed once. Quail cannot follow a similar plan with an ordinary LLM because the model state for each partner depends on the preceding anchor. Quail must recompute every partner for each new anchor. We want to train a specialized join model to encode each document independently and map matching documents to comparable representations. Quail could build an in-memory index from the *build side* representations, then find matches for each *probe side* representation without a full model forward pass for every document pair. Because the query planner may choose either relation as the *build side*, training should produce the same match score for `(left, right)` and `(right, left)`. One systems question is how to run and train many specialized models alongside a larger model on the same GPU.

More blog posts, and eventually a technical report, are coming soon. For now, please try Quail out! If any of the ideas above sound interesting, please reach out! We are hiring PhD students and postdocs.
