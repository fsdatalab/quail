...
**Train models for AI-SQL operators.** It would also be good to train specialized models for filters and joins. [Google's work on lightweight proxy models for AI-SQL](https://arxiv.org/abs/2603.15970) suggests small filter models can reduce cost substantially. For joins, you may have realized our current algorithm is a *KV-aware nested loop join*: Quail reuses each anchor's KV, but recomputes every partner for each new anchor. A specialized join model could instead use the same build and probe structure as a hash join. Quail would encode each row from the *build side* once and retain the representations in GPU memory. Quail would then encode each row from the *probe side* once and compare it with the stored build representations using a small scoring layer. Because the query planner may choose either relation as the build side, the model must give the same answer for `(left, right)` and `(right, left)`. One systems question is how to run and train many specialized models alongside a larger model on the same GPU.
