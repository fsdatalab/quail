...
**Use proxy models for query planner hints.** Proxy models need not be limited to evaluating predicates during execution. They could also provide planner hints. For example, proxy models could predict filter selectivities, which would help Quail choose the filter order. They could also predict which documents will reach later operators, which would help Quail decide which KV to retain. Errors in the proxy model would not affect query accuracy, which is nice.
