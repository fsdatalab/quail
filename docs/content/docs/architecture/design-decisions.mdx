...
The current runtime does not include `AI.CLASSIFY`, `AI.EXTRACT`, or
`AI.MAP`; they are on the roadmap. Open-ended generation, speculation,
and request forking are outside the current design.
...
Every session names its GPU count, model, backend, and device. Use
`backend="quail"` for normal execution. `gpu_timing` controls optional
measurement and remains off by default.
