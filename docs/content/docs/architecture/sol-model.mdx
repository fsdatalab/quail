...
`reports/make_sol_quailb.py` calls the estimator for the 32 default
queries at sf=0.1 on Qwen3 4B fp8 and Qwen3 32B fp8. The script supplies
only what the benchmark has: the corpora, the queries, and the saved
labels, wrapped as the `answer` callable. Document lengths and prompt
lengths come from the session's tokenizer.

The saved estimates at `/results/sol/sol_quailb_sf0.1.json` on the
`quail-results` volume predate the four-filter FEV-9 definition. Regenerate
them before comparing the current suite with those estimates.

The current plot inputs are saved at
`/results/sol/2026-09-06-quailb-prefix-reuse.json`. They preserve the other
31 estimates and replace FEV-9 with a CPU calculation for its four-filter
definition. This uses the estimate with prefix reuse across requests,
documents, and repeated aliases. For FEV-9 on 4B, it is 5.821 seconds,
