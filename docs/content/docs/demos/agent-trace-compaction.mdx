...
Both versions run the Quail engine in process. The demo does not submit
queries to a remote Quail Server.
...
We set `{'anchor': 'c'}` so Quail keeps the trajectory KV in GPU
memory while it streams the questions. Each question only matches one
trajectory because we join on `conversation_id`.
