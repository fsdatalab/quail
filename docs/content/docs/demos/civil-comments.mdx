...
Jev has no SQL planner. The demo selects one of two request schedules:
...
The combined schedule evaluates every field before removing comments that
fail the toxicity condition. The toxicity-first schedule applies the filter
first. In the measured run, 3,016 comments passed, so the remaining 6,984
comments required no field checks.
