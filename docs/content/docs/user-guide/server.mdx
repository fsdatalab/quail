...
run. It rejects the id with HTTP 409 and `QueryIdConflictError` if the
query, engine config, inputs, or timeout differ.
...
the row index, last stage asked, and result. A join entry arrives after an
anchor document finishes. `matches` contains the partner rows that
returned true. `asked` gives the number of evaluated pairs.
