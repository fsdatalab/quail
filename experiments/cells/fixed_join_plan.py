"""Compare fixed and adaptive FEV-9 plans on one Modal H100.

Prepare the baseline checkout and tee the invocation:

    git worktree add --detach /tmp/quail-fev9-baseline-b873066 b873066
    uv run modal run experiments/cells/fixed_join_plan.py \
      --prediction "State the prediction before running." \
      2>&1 | tee /tmp/quail-fixed-join-plan.log
...
baseline = Path(os.environ.get("QUAIL_BASELINE_DIR", "/tmp/quail-fev9-baseline-b873066"))
...
def compare(prediction: str) -> str:
    ...
    output = Path(f"/results/ablations/fixed-join-plan-{stamp}")
    output.mkdir(parents=True, exist_ok=True)
    gpu = subprocess.check_output([
        "nvidia-smi", "--query-gpu=uuid", "--format=csv,noheader",
    ], text=True).strip()
    (output / "setup.json").write_text(json.dumps({
        "prediction": prediction, "baseline_commit": "b873066",
        "gpu_uuid": gpu, "order": ["adaptive", "fixed"],
        "warmup": "One unmeasured FEV-9 run per implementation",
    }, indent=2))
    for label, python_path in (("adaptive", "/opt/quail-baseline:/root"), ("fixed", "/root")):
