r"""Compare materialized and streamed filter-to-join execution on one H100.

The baseline checkout is main before the streamed edge (8338d92): every
filter chain finishes for all documents before its join starts, so the
join's anchors come back from the retention pool or are recomputed. The
comparison is this branch: the first anchor's chain streams each passing
document into the join with its KV pinned. Both run IMDB-3, IMDB-4,
IMDB-5, IMDB-10, and BIO-3 in the same container on the same GPU, one
warmup and one measured run each.

Prepare the baseline checkout and tee the invocation:

    git worktree add --detach /tmp/quail-materialized-baseline-8338d92 8338d92
    uv run modal run experiments/cells/streamed_filter_join.py \
...
BASELINE_COMMIT = "8338d92"

baseline = Path(os.environ.get(
    "QUAIL_BASELINE_DIR", f"/tmp/quail-materialized-baseline-{BASELINE_COMMIT}"))
image = (
...
def compare(prediction: str, query_ids: str) -> str:
    ...
    output = Path(f"/results/ablations/streamed-filter-join-{stamp}")
    output.mkdir(parents=True, exist_ok=True)
    ...
    (output / "setup.json").write_text(json.dumps({
        ...
        "gpu_uuid": gpu, "order": ["materialized", "streamed"],
        "warmup": "One unmeasured run per query per implementation",
    }, indent=2))
    for label, python_path in (("materialized", "/opt/quail-baseline:/root"),
                               ("streamed", "/root")):
