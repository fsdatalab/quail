"""Profile all three SGLang FEV-9 joins with PyTorch Profiler on Modal.
    ...
    uv run modal run --detach experiments/profile_sglang_input.py \
      2>&1 | tee /tmp/quail-sglang-input-profile.log

Scheduler CUDA traces, driver CPU traces, answers, and input preparation timings
are saved on quail-results under /results/ablations/sglang-input-profile-<UTC>/.
...
from experiments.sglang_input_profile_worker import PREDICTION_TEXT
...
def profile():
    ...
    from experiments.sglang_input_profile_worker import profile_worker
    from quail.bench.process_isolation import _stop_process_group

    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    root = Path(f"/results/ablations/sglang-input-profile-{stamp}")
