...
\author{
  Anonymous Author(s)\\
  \textit{Draft of \today. Numbers in Section 2.5 are analytical estimates; measured results are forthcoming.}
}
\date{}
...
The intuition is concrete in Example~\ref{ex:contracts}. Every schedule must acquire every contract's KV in HBM at least once before its first evaluation, and with a cold corpus the only source is prefill, so $\mathrm{OPT}$ is lower-bounded by one corpus prefill; a 4K contract costs about 90\,ms of prefill compute at 4B scale. Cohorting pays exactly that floor plus $z$ cached passes, and one cached pass reads the contract's 288\,MiB of KV in about 0.12\,ms, so $\eps \approx 1.5 \times 10^{-3}$ and three filters add under half a percent. The datasheet approximation $\eps \approx (m/\beta_{\hbm})/(2P_{\Theta}/\mathrm{FLOPS})$ gives the same value, improves with scale since KV bytes per token grow sublinearly in parameters while prefill FLOPs grow linearly, and is invariant under tensor sharding: at degree $\nu$, per-device KV per token is $m/\nu$ while per-device compute per token is $2P_{\Theta}/(\nu\,\mathrm{FLOPS})$, so $\nu$ cancels, the guarantee holds per replica at any sharding degree, and the single-document hypothesis relaxes by the factor $g$.
...
\end{equation}
with $f_B, f_P$ monotone nondecreasing one-dimensional fits, $\beta_N \ge 0$, one global intercept $b_0 \ge 0$. Host preparation is $\widehat{T}_{\mathrm{host}} = h_0 + h_N N + h_A A$ with $A$ the KV block allocations; tier transfers are priced separately from measured bandwidths, because a PCIe copy and a block-table update differ by orders of magnitude and a lumped coefficient predicts neither.
