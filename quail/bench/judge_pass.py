...
PREDICTION = (
    "sf=0.1 samples 500 known LePaRD citation pairs, which produce "
    "500 destination excerpts, 433 cited passages and 216,500 exact "
    "join labels. Exactly 500 join labels should be TRUE. Qwen3 32B "
    "writes 2,933 LePaRD filter labels. The changed corpus gives every "
    "predicate a new label-set id, so all four workloads run again on "
    "four H100! GPUs, one workload per GPU. The run should finish "
    "without an out-of-memory failure at gpu_memory_utilization=0.85."
)
