...
def queries(sess):
    ...
    def bio7():
        ...
        qy = add_join(r1, m1, REACTION,
                      quail.col("r1.report"), quail.col("m1.term"))
        qy = add_join(qy, r2, REACTION_SEVERE,
                      quail.col("r2.report"), quail.col("m1.term"))
        qy = add_join(qy, m2, REACTION,
                      quail.col("r2.report"), quail.col("m2.term"))
        return qy.select(
            "r1.id", "m1.id", "r2.id", "m2.id", order="by_cost")
