...
come from? Figures: `plots/ablation_ladder.png` and
`plots/ablation_profile.png`.
...
packed configurations (`results/ablation_profile.json`, figure
`plots/ablation_profile.png`) shows why. GPU kernel time per token:

| work | stock vLLM fp8 (banked engine profile) | A2 | A3 |
|---|---|---|---|
| matrix multiplies | 5.52 µs | 5.18 µs | 5.46 µs |
| attention | 0.81 µs | 0.79 µs | 0.85 µs |
| small kernels (normalize, quantize, silu, rotate) | 4.00 µs | 4.22 µs | 1.87 µs |
| KV storage work | 0.43 µs | 1.08 µs | 0.63 µs |
| **total** | **10.77 µs** | **11.35 µs** | **8.91 µs** |
...
- The small kernels in A2 are vLLM's own, and they cost the same per
  token in our loop as inside the engine (4.22 vs 4.00 µs). No win
  there.
- What tips A2 under stock is the price of keeping KV our way:
  scattering each document's KV into pages and building the block
  tables that attention reads through, plus two extra copies per
  layer inside vLLM's unfused qk-norm path. That is 1.08 µs/token
  against the engine's 0.43 for a plain cache write. A2 also avoids
...
second drop, 2.5 microseconds per token of wall. The profile shows
2.8 µs/token of GPU work removed: 2.35 from fusing the small kernels
(four quantize passes per layer become one; normalize+add+quantize
and silu+multiply+quantize each become one pass) and 0.45 from the
...
- `reports/plots/ablation_ladder.png`,
  `reports/plots/ablation_profile.png` — generated by
  `reports/make_ablation_plots.py`.
- Logs: `results/ablation_packed4.log`, `results/ablation_stock_a0.log`,
  `results/ablation_stock_a1.log`, `results/ablation_profile.log`.
