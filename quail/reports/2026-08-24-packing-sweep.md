...
## Rejected rewrite: one a2

A later pass on this branch replaced `a*T + a2c*Sc + a2x*Sx` with
`a*T + a2*S + c + p*suffixes`, merging `S = Sc + Sx` and arguing
that FLOPs per attention pair are the same wherever the KV lives.
The CUDA event timing did not change. The rewrite is wrong on the
units, and the re-fit shows it.

`Sc` is `n²` per causal segment: the `/2` from triangular
self-attention is absorbed in `a2c`. `Sx` is `n * kept_context`:
every suffix token reads the whole kept KV, no `/2`. Equal cost
per attended token therefore means `a2x = 2 * a2c`, not
`a2x = a2c`. The joint fit already reported that:
...
The 2.26 ratio is the `/2` convention plus a 13% paged-attention
premium. It is not a 2.2x per-pair cost gap. Adding `Sc` and `Sx`
into one column and fitting one `a2` treats a causal `n²` count as
if it were a full rectangle. The fitted `a2` is then pulled up by
the join chunks (4B: 9.08e-10 against the filter `a2c` of 4.34e-10).

What that joint fit did to the filter data, the thing issue #25
asked to validate:

| | one-a2 joint fit | two-coefficient filter fit |
|---|---|---|
| 4B chunks used | 248, with 29 dropped | 24 of 25 |
| BIO-1 kept | 0 of 8 | 8 |
| IMDB-1 kept | 1 of 17 | 16 |
| BIO-1 residual | dropped; commit text said 12-17% overpredict | in the line |
| 4B `a` | 7.47 us/token | 7.867 us/token |
| 4B `a2` | 9.08e-10 | 4.343e-10 (`a2c`) |

The 5% outlier gate then classified the filter chunks as outliers
because the model was wrong, not because the measurements were.
The per-query IMDB-1 fit on the one remaining chunk produced
`a = -7e11` and `R² = -1.6e13`. The plot script still reads
`fits.filter.a2c` and `x_mean_ctx`, which the rewritten summary
no longer had, so the figures were not regenerated.

The FLOPs argument does not justify the merge. At these lengths on
one H100, attention is bandwidth-bound. Causal FlashAttention and
paged reads of non-contiguous arena pages move different bytes per
attended token. The measured 13% premium is that difference. A
per-suffix dispatch cost already sits in the two-coefficient model
(35 us at 4B, 114 us at 32B). The launch floor on chunks under
~2k tokens is ~1% of chain wall time and is handled by dropping
those tail chunks, not by adding a `c` the planner does not use.

If someone re-ran `calibrate.py` under the rewrite, the committed
`a2` would roughly double. `restore_crossover_tokens` solves
`kappa/bw = a + a2*h` in the filter convention. That doubled `a2`
would move the crossover even though no restore decision uses join
chunks.

This report keeps the two-coefficient fit. The planner still
commits `(a, a2c)` from the filter-only line.
