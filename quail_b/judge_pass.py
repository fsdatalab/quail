"""Build the sf=0.1 QUAIL-B reference labels on Modal, one GPU per workload.

The labeling itself is `quail_b.labeling`; this module only wraps
it in Modal functions that mount the results volume.

    uv run modal run -m quail_b.judge_pass

Reuse labels after an unrelated table changes in a new corpus:

    uv run modal run --detach -m quail_b.judge_pass \
