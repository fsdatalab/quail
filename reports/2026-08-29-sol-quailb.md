...
- The full result is at `/results/sol/sol_quailb_sf0.1.json` on the
  ...
  where every distinct token prefix in the query's scanned documents is
  computed once, across documents and across aliases of one column, and
  `per_document.sol_s`, where every alias's document is computed once and
  reused only across its own questions. The file also records, per corpus,
  how many tokens are a prefix another document has.
- Regenerated on 2026-09-05 with the cross alias credit: in a self join
  such as FEV-7 (claims x evidence x evidence), the second evidence alias
  reuses the prefixes the first one computed. Only the distinct prefix
  estimates of IMDB-9, IMDB-10, FEV-7, FEV-8, and FEV-9 changed.
...
nodes of the corpus prefix trie. A column scanned under two aliases is one
trie: an anchor row whose column another alias filtered, or whose row is
live under another alias that already holds its prefixes, pays only the
frame. Work is ideally packed across the whole
...
- The distinct prefix 4B estimates total 453.42 seconds over 32 queries;
  the per document ones total 632.66 seconds. On the 30 non agent queries
  the two are 358.09 and 370.34 seconds: the review, report, claim, and
  passage corpora share under 1% of their tokens as prefixes, and the
  rest of the gap is the five self join queries, where the second alias
  of a column reuses the first alias's prefixes (IMDB-9: 16.38 seconds
  distinct prefix, compared with 21.25 per document).
- The distinct prefix 32B estimates total 3,330.74 seconds; the per
...
- IMDB totals 97.99 seconds for 4B.
- BioDEX totals 116.14 seconds for 4B.
- FEVER totals 79.56 seconds for 4B.
