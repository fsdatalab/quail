...
Download the profile and its unprofiled reference from quail-results:
    ...
    uv run modal volume get quail-results \
      "$RUN_DIRECTORY/result.json" "$W/sglang/result.json"
    ...
    SGLANG_BASELINE=benchmarks/quailb/families/20260906T222629Z-sglang-suffix-major
    uv run modal volume get quail-results \
      "$SGLANG_BASELINE/fever-sglang-process.json" "$W/sglang/baseline.json"
    ...
    uv run modal volume get quail-results \
      "$VLLM_RUN/result.json" "$W/vllm/result.json"
    uv run modal volume get quail-results \
      "$INPUT_RUN/result.json" "$W/sglang-input/result.json"
    for join in 0 1 2; do
    ...
    VLLM_BASELINE=benchmarks/quailb/runs/qb_20260906T211500Z_37db8090
    VLLM_BASELINE_FILE=20260906T211500Z-quailb-sf0.1-lf1-qwen3-4b-fp8-families
    uv run modal volume get quail-results \
      "$VLLM_BASELINE/$VLLM_BASELINE_FILE-pipelined_vllm.json" "$W/vllm/baseline.json"
...
def load_profiles(root, backend="sglang"):
    ...
    for join in result["joins"]:
        ...
        window = tuple(driver["base_ns"] + round(value * 1000)
                       for value in (start, end))
        assert abs(window[0] - join["started_unix_ns"]) < 100_000_000
        assert abs(window[1] - join["finished_unix_ns"]) < 100_000_000
        filename = (f"join-{number}-TP-0.trace.json.gz" if backend == "sglang"
                    else "worker.trace.json.gz")
        scheduler = read_trace(root / f"join-{number}" / filename, window)
        assert scheduler["gpu_busy_us"] > 0
        scope = ("scheduler.get_next_batch_to_run" if backend == "sglang"
                 else "vllm.scheduler.schedule")
...
def plot_profiles(profiles):
    ...
    for (join, scheduler, _), (timeline, cpu) in zip(profiles, axes):
        ...
        bins = binned_activity(
            scheduler["gpu_busy_intervals"], scheduler["start_us"],
            scheduler["end_us"], 500_000,
        )
        ...
        cpu.barh(range(len(selected)), values,
                 color=[color for _, _, color in selected])
        ...
        for index, value in enumerate(values):
            cpu.text(value + max(values) * 0.02, index, f"{value:.2f}",
                     va="center", fontsize=10)
    figure.suptitle("FEV-9, SGLang PyTorch profile, Qwen3 4B FP8, one H100",
                    fontsize=17, y=0.975)
    note_text = (
        "GPU activity is the union of kernels and transfers in each 0.5-second "
        "bin. It is not hardware occupancy.\n"
        "CPU scopes measure elapsed time and may overlap GPU work.\n"
        "Join intervals exclude trace export. Profiling overhead remains; "
        "benchmark measurements are unchanged.")
    figure.text(0.07, 0.025, note_text, fontsize=10, linespacing=1.5)
    figure.subplots_adjust(left=0.07, right=0.96, top=0.91, bottom=0.13,
...
def summarize(result, baseline, profiles):
    ...
    for join, scheduler, driver in profiles:
        print(json.dumps({
            ...
            "initial_gpu_idle_s": (scheduler["gpu_busy_intervals"][0][0]
                                   - scheduler["start_us"]) / 1e6,
            ...
            "scope_gpu_idle_s": {k: v / 1e6
                                 for k, v in scheduler["scope_gpu_idle_us"].items()},
            "cpu_process_times": join.get("process_cpu"),
            "driver_scope_s": {k: v / 1e6 for k, v in driver["scope_us"].items()},
            "top_kernels": sorted(scheduler["kernels"].items(),
                                  key=lambda kv: kv[1]["us"], reverse=True)[:10],
            "top_cpu_ops": sorted(scheduler["cpu_ops"].items(),
                                  key=lambda kv: kv[1]["us"], reverse=True)[:15],
        }))
    method = result["methods"][0]
...
def plot_comparison(sglang_profiles, vllm_profiles):
    ...
    limit = max(trace["capture_us"] / 1e6
                for _, profiles, _ in groups for _, trace, _ in profiles)
    for column, (name, profiles, color) in enumerate(groups):
        for row, (join, trace, _) in enumerate(profiles):
            axis = axes[row, column]
            bins = binned_activity(trace["gpu_busy_intervals"], trace["start_us"],
                                   trace["end_us"], 500_000)
            times = [(left - trace["start_us"]) / 1e6 for left, _, _ in bins]
            heights = [100 * occupied / (right - left)
                       for left, right, occupied in bins]
            widths = [(right - left) / 1e6 for left, right, _ in bins]
            axis.bar(times, heights, width=widths, align="edge", color=color)
            share = 100 * trace["gpu_busy_us"] / trace["capture_us"]
            axis.set_title(f"FEV-9 join {join['join'] + 1}, {name}, "
                           f"GPU active {share:.1f}%")
            axis.set(xlabel="seconds", ylabel="percent", xlim=(0, limit),
                     ylim=(0, 105))
            axis.set_yticks([0, 25, 50, 75, 100])
    figure.suptitle("FEV-9 join profiles, Qwen3 4B FP8, one H100 per engine",
                    fontsize=16, y=0.975)
    note_text = (
        "Each bar shows time covered by GPU kernels or transfers within 0.5 "
        "seconds. Overlapping GPU work counts once.\n"
        "The traces include profiling overhead and exclude trace export. "
        "Each engine evaluates its own filter survivors.")
    figure.text(0.07, 0.025, note_text, fontsize=10, linespacing=1.5)
    figure.subplots_adjust(left=0.07, right=0.97, top=0.90, bottom=0.13,
...
def main(workdir):
    ...
    for join, trace, _ in inputs[2]:
        print(json.dumps({"input_join": join["join"],
                          **input_preparation_breakdown(join, trace)}))
