# Scan ring: KV retention that cannot starve admission
...
- `run_filter` (`quail/executor/loop.py`): before a filter with
  ...
  earlier operator's retained KV crowds the ring, evict the least
  valuable prefixes once, in bulk, up front. The old
...
- `RetainedPool` (`quail/executor/retention.py`): passing survivors
  ...
  with the least saved recompute per page only when its value
  strictly exceeds what the victims lose together, so total
  retained value only rises, equal value never swaps, and longer
  documents displace shorter ones. Replacement happens in the
  answer path (heap bookkeeping), never in admission.
...
retention competes only with itself, by value.
...
- Filter phase: 50.28 s in 2,916 forward passes (469 mean tokens)
  before; 14.54 s in 17 passes (103,484 mean tokens) after - the
