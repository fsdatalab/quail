...
- The registry stores ordinary Python objects. There is no extension manifest,
  parser, or registration replay. `load_extension` calls its module's
...
- The caller installs dependencies and includes extension code in its image.
  Module loading does not accept dependency arguments.
- Create the registry inside the Modal GPU function, including objects with
  connections or locks. The function collects or saves results before returning.
  See [the session update](2026-09-09-in-process-sessions.md).
