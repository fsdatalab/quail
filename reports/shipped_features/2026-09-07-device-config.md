...
Session arguments after `config` are keyword only. `QueryRequest` carries
hardware selection inside `config` for both local execution and Modal.
...
selection, planning, validation, and transfer to a worker.
