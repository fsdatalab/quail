...
- Two per-pair Python loops on Quail's side of scoring are now Arrow
  work: the runner's row-index-to-id mapping (`pc.take`) and the Quail
  backend's join answer table assembly in `quail/execution.py`, which
  visits anchors in Python and hands the pairs to Arrow through
  `itertools.chain`, `pa.array`, and one `pc.take` for the partner ids.
  Timed on a synthetic 2,000,000-pair join: 0.7 s to 0.1 s and 0.9 s to
  0.2 s. The September 12 run evaluated 3.15 million pairs
