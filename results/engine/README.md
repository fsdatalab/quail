...
| `persist_split7_10k.json`, `persist_split7_quail_waves_chain_10k.json` | the split7 protocol at 10,000 documents | the at-scale head-to-head: 3.29M tokens, 242.4 GB of KV against the 71 GB pool. Stock: write 47.44 s, restores 32.35/30.42 s in 20,002 per-document copies. Quail (chain + waves + serial scheduling at 0.90 utilization, both self-handicaps ours): write 50.90 s, restores 16.63/16.78 s against the predicted 10-18 s - 1.8-1.9x faster, moving the identical 522.5 GB with zero duplication in 200 planned copies at 47.3 GB/s. 952 living sessions slid 10,000 documents through the pool. Getting the chain side to run took the whole slot-management chain recorded in the commits: pool-derived session capacity, rewound sessions at the queue front, slack-gated fresh admissions, claimed documents budgeted while pending, serial scheduling, and the utilization step-down |
| `persist_split7_quail_waves_chain_3k.json`, `persist_xfer_trace_split7_quail_waves_chain_3k.jsonl.gz` | the 3k memory-pressure validation | 73.3 GB of KV against the 71 GB pool: write 17.30 s, restores 6.24/5.69 s, 157.9 GB loaded with ~8 percent reload overhead - the cell that validated the slot machinery before 10k |
