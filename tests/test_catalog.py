...
from quail.catalog import DocumentProvider, ScanRequest
...
def test_table_provider_scans_and_validation(monkeypatch):
    ...
    batches = list(provider.scan(ScanRequest(columns=("body",), limit=5, batch_rows=2)))
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: Dataset.from_dict({
            "id": ["a", "b"], "body": ["first", "second"], "unused": [1, 2]}))
    ...
    assert table.to_pydict() == {"id": ["a", "b"], "body": ["first", "second"]}
