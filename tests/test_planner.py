...
def _hand_spec(pos, aliases, anchor):
    """A full-join spec dict in search_joins's input form. No
    selectivity, so live counts never thin and orders differ only in
    residency credit."""
    return dict(written_pos=pos, aliases=list(aliases), anchor=anchor,
                anchor_free=False, semantics="full", selectivity=None,
                frame_tokens={a: 5 for a in aliases},
                label_tokens={a: 4 for a in aliases},
                tail_tokens=6)


def test_hold_check_denies_reused_anchor_credit():
    """An a-b-a order relies on a's KV staying resident through the
    b group. At pre=10 and 16-token pages, a's retained mass is
    3 x 112 = 336 tokens; the b group's worst tuple pins 475 more
    (one 400-token b document, its frame, one c partner). An arena
    of 700 tokens cannot hold both, so the later a group must price
    as scans, not 'kept'."""
    from quail.planner.joins import search_joins

    specs = [_hand_spec(0, ("a", "b"), "a"),
             _hand_spec(1, ("b", "c"), "b"),
             _hand_spec(2, ("a", "c"), "a")]
    live = {"a": 3.0, "b": 3.0, "c": 2.0}
    toks = {"a": [100] * 3, "b": [400] * 3, "c": [50] * 2}

    free = search_joins(specs, live, toks, {}, 10, 100_000,
                        QWEN3_4B_FP8, H100_SXM, fixed_order=True)
    assert [r["resident"] for r in free["records"]] == \
        ["none", "none", "kept"]

    tight = search_joins(specs, live, toks, {}, 10, 100_000,
                         QWEN3_4B_FP8, H100_SXM, fixed_order=True,
                         arena_tokens=700.0)
    assert [r["resident"] for r in tight["records"]] == \
        ["none", "none", "none"]
    # the denied stage scans its 3 anchor prefixes of 110 tokens
    assert tight["work"].tokens == pytest.approx(
        free["work"].tokens + 330)

    # order left free, the search avoids the overflow instead: the
    # two a groups run back to back, so holding a through b is never
    # required and one kept credit survives
    dp = search_joins(specs, live, toks, {}, 10, 100_000,
                      QWEN3_4B_FP8, H100_SXM, arena_tokens=700.0)
    anchors = [a for _, a in dp["seq"]]
    assert anchors in (["a", "a", "b"], ["b", "a", "a"])
    kept = [r["anchor"] for r in dp["records"]
            if r["resident"] == "kept"]
    assert kept == ["a"]


def test_hold_check_denies_filter_credit_over_capacity():
    """A filter keep the arena cannot hold beside the join's own
    tuple is not priced as resident."""
    from quail.planner.joins import search_joins
    ...
    free = search_joins(specs, live, toks, resident, 10, 100_000,
                        QWEN3_4B_FP8, H100_SXM)
    assert free["records"][0]["resident"] == "filter"

    # a's held mass is 336 tokens (3 page-rounded prefixes of 112)
    # and the worst tuple pins 175 more; 300 cannot hold both
    tight = search_joins(specs, live, toks, resident, 10, 100_000,
                         QWEN3_4B_FP8, H100_SXM, arena_tokens=300.0)
    assert tight["records"][0]["resident"] == "none"
    # the 3 resident prefixes of 110 tokens scan instead of ask
    assert tight["work"].tokens == pytest.approx(
        free["work"].tokens + 330)
